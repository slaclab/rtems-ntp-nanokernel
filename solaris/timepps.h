 ...
 */
#define PPS_CAP	(PPS_CAPTUREASSERT | PPS_TSFMT_TSPEC | PPS_TSFMT_NTPFP)
	...
	)
{
	...
	if (ioctl(filedes, TIOCSPPS, &nonzero))
		return (-1);
	...
	)
{
	...
	if (ioctl(handle->filedes, TIOCGPPSEV, &ev) < 0)
		return (-1);

	/*
	 * Apply offsets as specified. Note that only assert timestamps
	 * are captured by this interface. Further note that apparently
	 * in Solaris 2.7 the TIOCGPPSEV doesn't increment ev.serial,
	 * and doesn't return the seconds value.
	 */
